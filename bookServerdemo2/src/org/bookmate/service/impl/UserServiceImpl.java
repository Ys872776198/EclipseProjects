...
//	}
	...
	@Override
	public boolean editUser2(Integer id, String username, String password,  Integer newStatus,Integer recommendStatus) {
		User user = userDao.selectUserById(id);
		if ("".equals(username) || "".equals(password)) {
			return false;
		}
		...
		user.setUserUsername(username);
		user.setUserMessageStatus(0);
		user.setUserNewStatus(newStatus);
		user.setUserForumStatus(0);
		user.setUserRecommendStatus(recommendStatus);
		userDao.updateUser(user);
		return true;
	}
